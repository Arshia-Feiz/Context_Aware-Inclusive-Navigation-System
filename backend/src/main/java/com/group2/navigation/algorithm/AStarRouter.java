 ...
 * 
 * A* uses: f(n) = g(n) + h(n)
 * - g(n) = actual cost from start to node n (weighted by preferences + context)
 * - h(n) = heuristic estimate from n to goal (haversine distance)
 * 
 * Uses graph search (closed set) to avoid revisiting nodes in cyclic road networks.
 */
public class AStarRouter {

    private Graph graph;
    private ContextDataRepository contextRepo; // nullable, for live DB queries

    public AStarRouter(Graph graph) {
        this.graph = graph;
        this.contextRepo = null;
    }

    public AStarRouter(Graph graph, ContextDataRepository contextRepo) {
        this.graph = graph;
     ...
     * 
     * If a ContextDataRepository was provided, edge weights will query the DB
     * in real-time for crime, lighting, construction, and hospital data.
     * Otherwise, it uses whatever context data was pre-loaded onto the edges.
     * 
    ...
    public List<Node> findRoute(double startLat, double startLng,
        ...
        while (!openSet.isEmpty()) {
            ...
            for (Edge edge : current.getEdges()) {
                ...
                // Calculate cost: use live DB queries if available, otherwise use pre-loaded data
                double edgeCost;
                if (contextRepo != null) {
                    edgeCost = edge.getWeightedCost(prefs, contextRepo);
                } else {
                    edgeCost = edge.getWeightedCost(prefs);
                }

                double tentativeGScore = current.getGScore() + edgeCost;
